...
handle(St, {connect, Server}) ->
	...
	case catch(genserver:request(ServerAtom, Data)) of
		ok ->
	    	{reply, ok, NewState};
	    already_connected ->
	    	{reply, {error, nick_taken, "The nick is already in use!"}, St};
	    {'EXIT', _} ->
...
% Computes the function with the given argument Element
handle(St, {job, F, Element, ID}) ->
	ServerAtom = list_to_atom(St#client_st.server),
	Result = {ID, F(Element)},
	genserver:request(ServerAtom, {message, Result}),
	{reply, ok, St}.
